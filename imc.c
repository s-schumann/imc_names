/*
 * $Id: imc.c 5259 2009-02-07 08:24:50Z bogdan_iancu $
 *
 ...
 *  2009-02-01  added fullname support (s_schumann)
 */
...
static param_export_t params[]={
	...
	{"use_from_displayname",	INT_PARAM, &use_from_displayname},
	{0,0,0}
};
...
int add_from_db(void)
{
	...
	int i, j, flag;
	db_key_t mq_result_cols[5], mquery_cols[2];
	...
	mq_result_cols[3] = &imc_col_fullname;
	...
	for(i =0 ; i< r_res->n ; i++)
	{
		...
		for(j =0; j< m_res->n; j++)
		{
			...
			fullname.s = m_row_vals[3].val.str_val.s;
			fullname.len = strlen(fullname.s);

			LM_DBG("adding member: [name]=%.*s (%.*s) [domain]=%.*s"
					" in [room]= %.*s\n",name.len, name.s, fullname.len, fullname.s, domain.len,domain.s,
					room->uri.len, room->uri.s);

			member = imc_add_member(room, &name, &domain, &fullname, flag);
			if(member == NULL)
			{
...
static int mod_init(void)
{
	...
	if(use_from_displayname)
		LM_DBG("Using From display-name for sender id\n");
	else
		LM_DBG("Using name from DB for sender id\n");
...
static int imc_manager(struct sip_msg* msg, char *str1, char *str2)
{
	...
	if(pfrom->display.len) {
		LM_DBG("From Display-Name: %.*s\n", pfrom->display.len, pfrom->display.s);
		fullname = pfrom->display;
	} else {
		fullname.s = "unknown";
		fullname.len = strlen(fullname.s);
	}

	if(fullname.s[0] == '"' && fullname.s[fullname.len-1]== '"') {
		LM_DBG("Display-Name uses apostrophes, removing...\n");
		fullname.s = fullname.s + 1;
		fullname.len = fullname.len - 2;
		LM_DBG("Apostrophes removed: %.*s\n", fullname.len, fullname.s);
	}
	...
	if(body.s[0]== imc_cmd_start_char)
	{
		...
		case IMC_CMDID_CREATE:
			if(imc_handle_create(msg, &cmd, pfrom_uri, pto_uri, &fullname)<0)
			{
		...
		break;
		case IMC_CMDID_JOIN:
			if(imc_handle_join(msg, &cmd, pfrom_uri, pto_uri, &fullname)<0)
			{
	...
	if(imc_handle_message(msg, &body, pfrom_uri, pto_uri, &fullname)<0)
	{
...
void destroy(void)
{
	...
	mq_cols[4] = &imc_col_fullname;
	mq_vals[4].type = DB_STR;
	mq_vals[4].nul = 0;
	...
	for(i=0; i<imc_hash_size; i++) 
	{
		...
		while(irp)
		{
			...
			while(member)
			{
				...
				mq_vals[4].val.str_val = member->fullname;
				...
				if(imc_dbf.insert(imc_db, mq_cols, mq_vals, 5)<0)
				{
					LM_ERR("failed to insert  into table imc_rooms\n");
					return;
				}
																...
																void* param)
{
	...
	while(imp)
	{
		...
		if(node == NULL)
			goto error;
		node = add_mi_node_child(node_r, MI_DUP_VALUE, "FULLNAME",8, imp->fullname.s, imp->fullname.len);
		if(node == NULL)
			goto error;
		imp = imp->next;
	}
