/*
 * $Id: imc_mng.c 4472 2008-07-11 19:51:40Z bogdan_iancu $
 *
 ...
 * add member
 */
imc_member_p imc_add_member(imc_room_p room, str* user, str* domain, str* fullname, int flags)
{
	imc_member_p imp = NULL;
	int size;
	
	if(room==NULL || user == NULL || user->s==NULL || user->len<=0 || fullname == NULL || fullname->s==NULL || fullname->len<=0
			|| domain == NULL || domain->s==NULL || domain->len<=0)
	{
		LM_ERR("invalid parameters\n");
		return NULL;
	...
	size = sizeof(imc_member_t) + (user->len+domain->len+6)*sizeof(char) + fullname->len*sizeof(char);
	...
	LM_DBG("[domain]= %.*s\n", imp->domain.len, imp->domain.s);
	imp->fullname.len = fullname->len;
	imp->fullname.s = (char*)(((char*)imp) + sizeof(imc_member_t)) + (user->len+domain->len+6)*sizeof(char);
	memcpy(imp->fullname.s, fullname->s, fullname->len);

	LM_DBG("[fullname]= %.*s\n", imp->fullname.len, imp->fullname.s);
	imp->flags  = flags;
 ...
 * rename member
 */
imc_member_p imc_rename_member(imc_room_p room, str* user, str* domain, str* fullname)
{
	imc_member_p imp = NULL;
	unsigned int hashid;
	...
	if(room==NULL || user == NULL || user->s==NULL || user->len<=0
			|| domain == NULL || domain->s==NULL || domain->len<=0 
			|| fullname == NULL || fullname->s==NULL || fullname->len<=0)
	{
		LM_ERR("invalid parameters\n");
		return NULL;
	}
	...
	while(imp) {
		...
		{
			LM_DBG("found member\n");
			LM_DBG("New name should be %.*s\n", fullname->len, fullname->s);
			LM_DBG("OLD: [fullname]= %.*s\n", imp->fullname.len, imp->fullname.s);
			imp->fullname.len = fullname->len;
			imp->fullname.s = (char*)(((char*)imp) + sizeof(imc_member_t)) + (imp->user.len+imp->domain.len+6)*sizeof(char);
			memcpy(imp->fullname.s, fullname->s, fullname->len);

			LM_DBG("NEW: [fullname]= %.*s\n", imp->fullname.len, imp->fullname.s);
			return imp;
		}
		imp = imp->next;
	}
