/*
 * $Id: imc_cmd.c 5016 2008-11-23 15:28:02Z saguti $
 *
		...
		struct sip_uri *src, struct sip_uri *dst, str *fullname)
{
	...
	if(room== NULL)
	{
		...
		member= imc_add_member(room, &src->user, &src->host, fullname, flag_member);
		if(member == NULL)
		{
			LM_ERR("failed to add owner [%.*s]\n", src->user.len, src->user.s);
			goto error;
		}
		LM_DBG("added the owner (%.*s) as the first member "
				"[%.*s]\n", member->fullname.len, member->fullname.s, member->uri.len, member->uri.s);
		...
		struct sip_uri *src, struct sip_uri *dst, str *fullname)
{
	...
	if(!(room->flags & IMC_ROOM_PRIV))
	{
		...
		{					
			...
			LM_DBG("member [%.*s] (%.*s) is in room already\n",
				member->uri.len,member->uri.s, member->fullname.len, member->fullname.s );
		}
	} else {
		if(member==NULL)
...
build_inform:
	/* send info message */
	body.s = imc_body_buf;
	body.len = snprintf(body.s, IMC_BUF_SIZE, "*** [%.*s] (%.*s) has joined the room",
					member->uri.len, member->uri.s, member->fullname.len, member->fullname.s);
	if(body.len>0)
		imc_room_broadcast(room, &imc_hdr_ctype, &body);
		...
		struct sip_uri *src, struct sip_uri *dst)
{
	...
	while(imp)
	{
		...
		p += imp->uri.len;
		/* S: add fullname display in brackets behind URI */
		*p++ = ' ';
		*p++ = '(';
		strncpy(p, imp->fullname.s, imp->fullname.len);
		p += imp->fullname.len;
		*p++ = ')';
		/* E */
		*p++ = '\n';
		...
		struct sip_uri *src, struct sip_uri *dst)
{
	...
	imc_rename_member(room, &src->user, &src->host, &cmd->param[0]);
	
	LM_DBG("Name renamed to [%.*s]\n", strlen(cmd->param[0].s), cmd->param[0].s);
	imc_release_room(room);
	return 0;
